# Purpose: This script aligns a Revit Scope Box to a selected Reference Plane.
...
if not error_message:
    try:
        ...
        if not bbox or not bbox.Min or not bbox.Max or bbox.Min.IsAlmostEqualTo(bbox.Max):
...
if not error_message:
    try:
        ...
        move_distance = 0.0
        # Check if the plane actually intersects the bounding box
        if min_dist <= TOLERANCE and max_dist >= -TOLERANCE: # Plane intersects or touches the box
        ...
        elif max_dist < -TOLERANCE: # Box is entirely on the negative side of the plane
            move_distance = -max_dist # Move the max point (closest point) onto the plane
        elif min_dist > TOLERANCE: # Box is entirely on the positive side of the plane
            move_distance = -min_dist # Move the min point (closest point) onto the plane
        else:
             # This case should not happen given the checks above
             error_message = "# Error: Could not determine relative position of Scope Box and Plane."
