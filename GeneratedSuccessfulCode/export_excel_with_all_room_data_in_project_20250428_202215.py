# Purpose: This script exports a list of all placed rooms in the project, including common data, to an Excel file (CSV format).
...
csv_lines.append('"Level","Room Number","Room Name","Area (sq ft)","Perimeter (ft)","Volume (cu ft)","Unbounded Height (ft)","Base Offset (ft)","Limit Offset (ft)"')
        ...
        if val_str is None or val_str == "":
             try:
                 ...
                 if storage_type == System.StorageType.Double:
                     # Format doubles nicely using AsString or fallback to str
                     formatted_double = param.AsString()
                     val_str = formatted_double if formatted_double else str(param.AsDouble())
                     ...
                     val_str = str(param.AsInteger())
                      ...
                      if element_id is not None and element_id != ElementId.InvalidElementId:
                          ...
                          val_str = linked_element.Name if linked_element else element_id.ToString()
                      else:
                          val_str = default
                 else:
...
for element in collector:
    ...
    if isinstance(element, Room):
        ...
        try:
            ...
            # Unplaced rooms have Area = 0 or very close to 0. Also check Location.
            area_internal = room.Area
            is_placed = room.Location is not None and area_internal > 1e-6

            # Only process placed rooms
            if is_placed:
                # --- Get Level Name ---
                level_name = "N/A"
                level_id = room.LevelId # Use LevelId first
                ...
                volume_str = "N/A (Check Vol Computations)" # Default value
                ...
                unbounded_height_internal = get_parameter_value_double(room, BuiltInParameter.ROOM_HEIGHT, 0.0) # ROOM_HEIGHT often represents Unbounded Height
                unbounded_height_str = System.String.Format("{0:.2f}", unbounded_height_internal)

                # --- Get Base Offset ---
                base_offset_internal = get_parameter_value_double(room, BuiltInParameter.ROOM_LOWER_OFFSET, 0.0)
                base_offset_str = System.String.Format("{0:.2f}", base_offset_internal)

                # --- Get Limit Offset ---
                limit_offset_internal = get_parameter_value_double(room, BuiltInParameter.ROOM_UPPER_OFFSET, 0.0)
                limit_offset_str = System.String.Format("{0:.2f}", limit_offset_internal)
                ...
                csv_line = ",".join([
                    ...
                    escape_csv(unbounded_height_str),
                    escape_csv(base_offset_str),
                    escape_csv(limit_offset_str)
                ])
...
if processed_count > 0:
    ...
    print("EXPORT::EXCEL::all_room_data_report.xlsx")
    print(file_content)
else:
