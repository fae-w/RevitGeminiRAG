    ...
    if template_id == ElementId.InvalidElementId:
        ...
        if not template_view or not isinstance(template_view, View):
            ...
            if solid_fill_pattern_id == ElementId.InvalidElementId:
                ...
                # Optionally reset background pattern ID and color explicitly
                ...
                # The script runs within an external transaction (C# wrapper), no transaction management needed here.
                try:
                    template_view.SetCategoryOverrides(walls_category_id, override_settings)
                    # print(f"# Successfully updated Wall cut pattern overrides in View Template: {{template_view.Name}}") # Optional success message (escaped)
